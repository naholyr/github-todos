...
function createIssue (repo, todo, conf) {
  ...
  function create () {
    return getCommentText(repo, todo, conf).then(function (text) {
      return service(conf.service).createIssue(repo, todo.title, text, [todo.label]).then(function (issue) {
        return issue;
      });
    });
  }
...
function getCommentText (repo, todo, conf) {
  // Link to file
  var url = service(conf.service).getFileUrl(repo, todo.file, todo.sha, todo.line);
  ...
  var comment = git.dir(path.join("..", todo.file))
    .then(fs.readFileStrict)
    .then(generateCommentText);

  var ref = Promise.resolve(url).then(function (url) {
    return "Ref. [" + todo.file + ":" + todo.line + "](" + url + ")";
  });

  return Promise.all([ref, comment]).spread(function (ref, comment) {
    return ref + comment;
  });
}
