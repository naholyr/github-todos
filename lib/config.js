...
function list (cb) {
  git.run("config --get-regexp " + PREFIX + ".*", function (err, stdout) {
    if (err) {
      return cb(err);
    }
...
function unset (option, cb) {
  var ops = [_unset(option, "--local")];
  if (_.contains(module.exports.globals, option)) {
    ops.push(_unset(option, "--global"));
  }
...
function set (option, value, cb) {
  var arg = _.contains(module.exports.globals, option) ? "--global" : "--local";
  git.run("config " + arg + " " + PREFIX + "." + option + " " + value, function (err /*, stdout */) {
    cb(err || null);
  });
}
