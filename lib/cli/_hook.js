...
function analyzeRange (range, targetSha, conf) {
  return git.run("diff -u " + range).then(function (diff) {
    var filesPatterns = conf.files;
    if (!_.isArray(filesPatterns)) {
      filesPatterns = filesPatterns.split(",");
    }
    filesPatterns = _.invoke(filesPatterns, "trim");
    ...
    return analyzeDiff(_.filter(parseDiff(diff), function (file) {
      ...
      var ignored = isIgnored(file.to, filesPatterns);
      if (ignored === null) {
        // Could not decide: hardcoded default = safe = ignore file
        ignored = true;
      }
...
function isIgnored (filename, patterns) {
  // from right to left
  return _.reduceRight(patterns, function (ignored, pattern) {
    if (ignored !== null) {
    ...
    // negative pattern?
    if (pattern[0] === "-") {
      if (minimatch(filename, pattern.substring(1).trimLeft())) {
        return true; // ignore file
      }
    } else {
      if (minimatch(filename, pattern)) {
        return false; // accept file
      }
    }
  ...
  var options = _.merge({
    "onProgress": function onProgress (err, result, todo) {
      if (err) {
        ...
        console.log("[Github-Todos] Added comment to issue #%s (%s) - %s", result.issue, result.title, result.url);
      } else if (result && result.type === "issue") {
        console.log("[Github-Todos] Created issue #%s (%s) - %s", result.number, result.title, result.url);
      } else if (!result) {
