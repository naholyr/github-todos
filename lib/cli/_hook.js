...
function checkBranch (branches, cb) {
  ...
  return Promise.promisify(git.currentBranch)().then(function (branch) {
...
function main (argv, conf) {
  ...
  if (argv.range) {
    ...
    return Promise.promisify(git.run)("rev-parse '" + targetCommit + "'").then(function (sha) {
...
function analyzeRange (range, targetSha, conf) {
  return Promise.promisify(git.run)("diff -u " + range).then(function (diff) {
    var allowedFiles = conf.files;
    if (!_.isArray(allowedFiles)) {
      allowedFiles = _.invoke(allowedFiles.split(","), "trim");
    }

    return Promise.promisify(analyzeDiff)(_.filter(parseDiff(diff), function (file) {
      if (!file.to) {
...
// String, Sha → void
function analyzeDiff (diff, sha, conf, cb) {
