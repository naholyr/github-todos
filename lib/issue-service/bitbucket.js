...
function get (path, options) {
  return getP(_.merge({ url: HOST + "/api/" + path }, options || {})).spread(function (res, body) {
...
function post (path, data, options) {
  return postP(_.merge({url: HOST + "/api/" + path, json: data}, options || {})).spread(function (res, body) {
    return body;
  });
}
...
function allIssues (oauth, repo) {
  return get("1.0/repositories/" + repo + "/issues", { "oauth": oauth }).map(convertIssue);
}
...
function saveOAuth (consumer) {
  return config.set("bitbucket.key", consumer.key)
    .then(_.partial(config.set, "bitbucket.secret", consumer.secret))
    .then(_.partialRight(_.pick, ["key", "secret"]));
}
