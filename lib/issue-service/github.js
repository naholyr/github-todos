...
function getFileUrl (repo, path, sha, line) {
  return config.list().then(function (conf) {
    var host = githubOption(conf, "host");
    var root = githubOption(conf, "url");
    var secure = githubOption(conf, "secure");

    var url = (secure ? "https" : "http") + "://" + host + root + "/blob/";
